...
public class QuadraticProbingHashing<T extends Comparable<T> & Serializable> implements MyHashing<T> {
	...
	public QuadraticProbingHashing(Class<T> clazz, int hashTableSize) {
	...
	public void insert(T value) {
		if (getCurrentLoadFactor() > maxLoadFactor) {
			...
			int counter = 0; //used for quadratic polynomial
			...
			for (int firstAvailableIndex = indexFromHashFunction; firstAvailableIndex < indexFromHashFunction + hashTable.length; firstAvailableIndex++) {
				int newIndex = (firstAvailableIndex + (counter * counter)) % hashTable.length; //to go to 0 index from max since we loop untill lenght+indexFromHashFunction
	...
	public boolean find(T value) {
		...
		for (int i = indexFromHashFunction; i < indexFromHashFunction + hashTable.length; i++) {
			int newIndex = (i + (counter * counter)) % hashTable.length;  //to go to 0 index from max since we loop untill lenght+indexFromHashFunction
